...
class GeoPriorResult:
    ...
    geo_prefix_weight_ratio: np.ndarray
...
def _build_dsxy_support(
    ...
    *,
    ...
    return Support(
        ...
        np.full(merged.shape[0], bool(is_query_prefix), dtype=bool),
    )
...
def _predict_idw(
    ...
    train_is_query_prefix: np.ndarray,
    query_xy: np.ndarray,
    ...
    # Keep this a pure support-geometry diagnostic; Tukey residual weights are
    # target-bearing and method-specific.
    neighbor_is_query_prefix = np.asarray(train_is_query_prefix, dtype=bool)[idx]
    prefix_weight_ratio = (weights * neighbor_is_query_prefix).sum(axis=1)
    ...
    diagnostics = {
        ...
        "geo_prefix_weight_ratio": prefix_weight_ratio.astype(np.float32),
    }
...
def _concat_support(
    ...
    if extra_support is not None and extra_support.xy.shape[0] > 0:
        ...
        prefix_flag_parts.append(extra_support.is_query_prefix)
    for well_id in selected_wells:
        ...
        prefix_flag_parts.append(support.is_query_prefix)
    if not xy_parts:
    ...
    is_query_prefix = np.concatenate(prefix_flag_parts).astype(bool, copy=False)
    ...
    if max_points > 0 and xy.shape[0] > max_points:
        ...
        is_query_prefix = is_query_prefix[keep]
    return Support(xy, y, path_dxy, is_query_prefix)
...
def _build_prior_for_query_well(
    ...
    geo_prefix_weight_ratio = np.full(query_well.md.shape[0], np.nan, dtype=np.float32)
    ...
    geo_prefix_weight_ratio[: query_well.suffix_start] = 0.0
    ...
    geo_prefix_weight_ratio[raw_idx] = _upsample_bin_values(
        raw_idx,
        bin_row_pos,
        bin_diagnostics["geo_prefix_weight_ratio"],
        cfg,
    )
    prev_idx = raw_idx - 1
...
def make_geo_prior_from_loaded_wells(
    support_data: dict[str, GeoWell],
    query_data: dict[str, GeoWell],
    support_wells,
    query_wells,
    cfg: GeoPriorConfig,
    *,
    exclude_query_from_support: bool,
    support_cache: dict[str, Support] | None = None,
) -> tuple[dict[str, GeoPriorResult], dict]:
    """Build analysis priors while reusing parsed wells and suffix support."""
    support_wells = [str(well_id) for well_id in support_wells]
    query_wells = [str(well_id) for well_id in query_wells]
    missing_support = [well_id for well_id in support_wells if well_id not in support_data]
    missing_query = [well_id for well_id in query_wells if well_id not in query_data]
    if missing_support:
        raise KeyError(f"support_data is missing wells: {missing_support[:10]}")
    if missing_query:
        raise KeyError(f"query_data is missing wells: {missing_query[:10]}")

    if support_cache is None:
        support_cache = {
            well_id: build_suffix_support(support_data[well_id], cfg)
            for well_id in support_wells
        }
    missing_cache = [well_id for well_id in support_wells if well_id not in support_cache]
    if missing_cache:
        raise KeyError(f"support_cache is missing wells: {missing_cache[:10]}")
    ...
    for query_well_id in query_wells:
        if exclude_query_from_support:
            active_support_wells = [
                well_id for well_id in support_wells if well_id != query_well_id
            ]
        else:
            active_support_wells = support_wells
        active_centroids = np.asarray(
            [support_centroids[well_id] for well_id in active_support_wells],
            dtype=np.float64,
        )
    ...
    score = _score_prior(query_data, prior, query_wells)
    summary = {
        "method": cfg.method,
        "config": cfg.to_dict(),
        ...
        "rmse": None if score is None else float(score),
    }
