...
def parse_args():
    ...
    parser.add_argument(
        "--pf-cache-dir",
        type=Path,
        help="Optional immutable PF-cache root shared by fixed-seed repeats.",
    )
    parser.add_argument(
        "--prepare-pf-only",
        action="store_true",
        help="Build/validate the fold-invariant train PF cache, then exit before model training.",
    )
    parser.add_argument(
        "--use-xy-neighbor",
        action="store_true",
...
def main():
    ...
    cfg.PF_heatmap_cache_dir = (
        args.pf_cache_dir.resolve() if args.pf_cache_dir is not None else cfg.output_dir / "pf_heatmap_cache"
    )
    cfg.PF_sample_cache_dir = cfg.output_dir / "pf_sample_cache"
    # The public recipe retains future TVT in its train PF cache and may use it
    # to shift PF features. Disable both paths for canonical Geo5 OOF.
    cfg.PF_heatmap_cache_include_target_labels = False
    cfg.PF_allow_target_tvt_feature = False
    ...
    if args.prepare_pf_only:
        cache_dir = seq_NN_train.prepare_pf_cache_if_needed(
            "train", cfg.train_path, list(well_ids), cfg, print
        )
        manifest = {
            "mode": "prepare_pf_only",
            "repeat": args.repeat,
        ...
        (cfg.output_dir / "pf_cache_manifest.json").write_text(json.dumps(manifest, indent=2) + "\n")
