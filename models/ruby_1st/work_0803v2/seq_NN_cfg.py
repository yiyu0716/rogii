...
class CFG:
    ...
    sim_cfg={
        ...
        'z_shift':{
            ...
            'nan_mode':{'shift':1.0}, # 'same'/'none'/'shift'/'global' or weighted mapping
            'TVT_bias_std':0.95,
    ...
    # The archived 0801 V2 cfg changed only the active U-Net channels after
    # construction. Keep its inactive two-stage fields byte-for-byte
    # equivalent; make_two_stage_unet_cfg() recomputes them when selected.
    two_stage_shared_channels = _channels_except(
        tuple(name for name in unet_static_channels if name != "geo_tvt_diff") + ("z_diff",),
        two_stage_tw_channels,
        two_stage_h_channels,
    )
    unet_cfg = {
...
def make_seq_requested_ablation_cfgs():
    ...
    return _validate_seq_train_cfgs(
        [
            ("submit_ver_0803_V1", submit_v1_cfg),
